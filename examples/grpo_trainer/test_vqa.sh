#!/bin/bash
set -x
export VLLM_ATTENTION_BACKEND=XFORMERS
export VLLM_USE_V1=0
# export wandb_key=25c95cfb8dfe322ae6d944a369d2ae63b65d9ece
# MODEL_PATH=/home/chendong/video-rl/Temporal-R1/Temporal-R1-3B-Charades
MODEL_PATH=/home/chendong/video-rl/Temporal-R1/checkpoints/easy_r1/qwen2_5_vl_7b_tvg_hf
# MODEL_PATH=/root/Temporal-R1-3B-Charades
VISIBLE_DEVICES="0,1,2,3"
# Assuming the modified YAML is named grpo_example.yaml in the config directory
# If you renamed it, use --config-name=your_new_name
CUDA_VISIBLE_DEVICES=${VISIBLE_DEVICES} python3 -m verl.trainer.main_vqa \
    data.train_files=/home/chendong/video-rl/charades_sta/charades_vqa_train.jsonl \
    data.val_files=/home/chendong/video-rl/charades_sta/test1.jsonl \
    +data.video_base_path=/home/chendong/video-rl/charades_sta/Charades_v1_480 \
    data.max_prompt_length=12284 \
    data.max_response_length=1024 \
    data.train_batch_size=4 \
    data.shuffle=true \
    data.filter_overlong_prompts=false \
    actor_rollout_ref.model.path=${MODEL_PATH} \
    actor_rollout_ref.actor.strategy=fsdp \
    actor_rollout_ref.actor.ppo_mini_batch_size=4 \
    actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu=1 \
    actor_rollout_ref.actor.entropy_coeff=1e-3 \
    actor_rollout_ref.actor.use_kl_loss=true \
    actor_rollout_ref.actor.kl_loss_coef=1e-2 \
    actor_rollout_ref.actor.kl_loss_type=low_var_kl \
    actor_rollout_ref.actor.ppo_epochs=1 \
    actor_rollout_ref.rollout.name=vllm \
    actor_rollout_ref.rollout.n=1  \
    actor_rollout_ref.rollout.top_k=-1  \
    actor_rollout_ref.rollout.top_p=1.0  \
    actor_rollout_ref.rollout.do_sample=true \
    actor_rollout_ref.rollout.enforce_eager=false \
    actor_rollout_ref.rollout.enable_chunked_prefill=false  \
    actor_rollout_ref.rollout.tensor_model_parallel_size=1 \
    actor_rollout_ref.rollout.gpu_memory_utilization=0.5 \
    critic.ppo_max_token_len_per_gpu=16384 \
    critic.rollout_n=1 \
    trainer.project_name=video_vqa \
    trainer.experiment_name=qwen2_5_vl_7b_vqa_reward_451_0605_rpp \
    trainer.nnodes=1 \
    trainer.n_gpus_per_node=4 \
    +trainer.grounding_weight=0.4 \
    +trainer.qa_accuracy_weight=0.5 \
    +trainer.qa_format_weight=0.1 \
    trainer.val_before_train=False \
    +trainer.val_only=False \
    trainer.resume_mode=disable \
    trainer.save_freq=200 \
    trainer.logger='["console", "wandb"]' 2>&1 | tee tvg.log
