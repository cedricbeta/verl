...
python3 -m recipe.dpo.main_dpo \
    ...
    actor_rollout_ref.actor.fsdp_config.wrap_policy.transformer_layer_cls="['Qwen2MLP', 'Qwen2Attention']" \
    ...
    algorithm.beta=0.1 \
    algorithm.loss_type=sigmoid \
    algorithm.use_kl_in_reward=True \
    algorithm.kl_penalty=kl \
    algorithm.kl_ctrl.kl_coef=0.001 \
    reward_model.model.path=$model_path \
    reward_model.model.ref_path=$model_path \
    reward_model.model.tokenizer_path=$model_path \
    reward_model.model.trust_remote_code=True \
    reward_model.micro_batch_size_per_gpu=1 \
    reward_model.model.update=before \
    reward_model.model.beta_train=0.1 \
    reward_model.model.loss_type=sigmoid \
    reward_model.model.optim.lr=1e-6 \
    reward_model.model.optim.grad_clip=10.0 \
    reward_model.model.fsdp_config.wrap_policy.transformer_layer_cls="['Qwen2MLP', 'Qwen2Attention']" \
    reward_model.model.input_tokenizer=null \
    reward_model.mini_batch_size=64 \
    trainer.val_before_train=False \
    trainer.logger=['console','wandb'] \
    trainer.project_name='dpo_example' \
    trainer.experiment_name='Qwen2.5-7B-DPO-gsm8k' \
