...
from .dpo_core_algos import compute_dpo_accuracy

logger = logging.getLogger(__file__)
logger.setLevel(os.getenv('VERL_PPO_LOGGING_LEVEL', 'WARN'))


class DPORewardModelWorker(Worker):
    """
    Worker for handling DPO reward model operations with FSDP support.
    """
    ...
    def _build_reward_ref_model_optimizer(self, config):
        """
        Build the reward model, reference model, and optimizer.
        """
        ...
        # Handle model configuration overrides
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # Convert reference model to specified precision
            ref_module.to(torch_dtype)

        # Wrap reward module with FSDP
        ...
        # Wrap reference module with FSDP
    ...
    def init_model(self):
        """
        Initialize the reward and reference models.
        """
        # Import external libraries if specified
        import_external_libs(self.config.model.get('external_lib', None))

        from .dpo_dp_rm import DataParallelDPORewardModel
        ...
        self.rm = DataParallelDPORewardModel(config=self.config,
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            # Calculate DPO accuracy metrics if preferences available
            if 'preferences' in data.batch:
                prompt_length = data.batch['prompts'].shape[-1]
                response_mask = data.batch['attention_mask'][:, prompt_length:]
                preferences = data.batch['preferences']
                
                dpo_acc = compute_dpo_accuracy(rm_scores, preferences, response_mask=response_mask, n_samples=data.meta_info['n'])
    ...
    def update_rm(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if 'preferences' in data.batch:
                ...
                dpo_acc = compute_dpo_accuracy(rm_scores, 
                                              preferences, 
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        """
        Save checkpoint for the reward model.
        """
        import torch
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.reward_module)
    ...
    def load_checkpoint(self, local_path, del_local_after_load=True):
        """
        Load checkpoint for the reward model.
        """
        import torch
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.reward_module)
